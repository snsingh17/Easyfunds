...
if(isset($_POST['review']))
{
	...
	$row=$id;
	 $sql1="SELECT * FROM campaigns WHERE campaign_id=$id";
	  ...
	  if (isset($_POST['accept'])) {
	  	...
	  	$approval='1';
	  	$sql2="SELECT campaignCreator FROM campaigns WHERE campaign_id='$id'";
	  	$result2=mysqli_query($conn,$sql2);
    while( $row2 = mysqli_fetch_assoc( $result2 ) ) 
            {
                $cc=$row2['campaignCreator'];
            }
            $sql3="SELECT organizer_email FROM organizers WHERE organizer_username='$cc'";
	  	$result3=mysqli_query($conn,$sql3);
    while( $row3 = mysqli_fetch_assoc( $result3 ) ) 
            {
                $email=$row3['organizer_email'];
            }
            $url1 = "https://".$url."/login.php";
                $to = $email;
                $subject = 'Campaign Accepted';
                $message = '<p>Hello, your campaign '.$campaignName.' has been successfully accepted:</br>';
                $message .= '<a href="' .$url1. '">Click to login</a></p>';
	  	...
	  	if($result1)
	  	{
	  	    ...
	  	    echo "<script>alert('Campaign Accepted Successfully and an automated email has been sent to notify organizer');</script>";
    ...
    while( $row3 = mysqli_fetch_assoc( $result3 ) ) 
            ...
            $url1 = "https://".$url."/login.php";
                ...
                $message = '<p>Hello, your campaign '.$campaignName.' has been deleted due to some errors,please talk with admin for more information or reply to this email. :</br>';
                $message .= '<a href="' .$url1. '">Click to login</a></p>';
        ...
        $sql="DELETE FROM `campaigns` WHERE campaign_id=$id ";
	  	...
	  	if($result1)
	  	{
	  	    ...
	  	    echo "<script>alert('Campaign Deleted Successfully and an automated mail to organizer have been sent');</script>";
                echo "<script>document.location ='manage-campaigns.php';</script>";
	  	}
          ...
          <div class="col-md-8">
            <div class="card card-user">
              <div class="card-header">
                <h5 class="card-title">Reveiw Campaigns</h5>
              </div>
              <div class="card-body">
                  <?php
                  ...
                  <div class="row">
                    <div class="col-md-12">
                      <div class="form-group">
                        ...
                        <textarea class="form-control textarea" name="campaignDescription" value="<?php echo $row['campaign_description'];?>"><?php echo $row['campaign_description'];?></textarea>
                      </div>
                    </div>
                  </div>
